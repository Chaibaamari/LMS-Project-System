...
function Settings() {
    ...
    const userData = useSelector((state: RootState) => state.employees.employees) 
    ...
    useEffect(() => {
        const SendUserData = async () => {
            ...
            dispatch(UserActions.FetchDataUsers(data.employes))
        }
        SendUserData()
    ...
    useEffect(() => {
        if (IsVisible) {
            setTimeout(() => {
                dispatch(UserActions.ClearNotificationDelete());
            }, 5000)
        }
    }, [dispatch, IsVisible]);
